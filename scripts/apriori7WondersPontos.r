...
#matriz de presença
data_1st_list_p <- data.frame(matrix(nrow = nrow(data_1st_t), ncol = length(head)))
...
colnames(data_1st_list_p) <- head
colnames(data_1st_list_m) <- head
colnames(data_1st_list_1) <- head
colnames(data_1st_list_3) <- head

data_nowinners_list_p <- data.frame(matrix(nrow = nrow(data_nowinners_t), ncol = length(head)))
...
colnames(data_nowinners_list_p) <- head
...
#param
data_1st_list_p[, "resultado"]  <- 1
data_1st_list_p$militar_vitoria <- as.integer(as.integer(as.character(data_1st_t$Military_Conflicts_Victory)) >= param)
data_1st_list_p$militar_derrota <- as.integer(as.integer(as.character(data_1st_t$Military_Conflicts_Defeat)) >= param)
data_1st_list_p$tesouro         <- as.integer(as.integer(as.character(data_1st_t$Treasury_Contents)) >= param)
data_1st_list_p$wonder          <- as.integer(as.integer(as.character(data_1st_t$Wonder)) >= param)
data_1st_list_p$civil           <- as.integer(as.integer(as.character(data_1st_t$Civilian_Structures)) >= param)
data_1st_list_p$cientifica      <- as.integer(as.integer(as.character(data_1st_t$Scientific_Structures)) >= param)
data_1st_list_p$comercial       <- as.integer(as.integer(as.character(data_1st_t$Commercial_Structures)) >= param)
data_1st_list_p$guilda          <- as.integer(as.integer(as.character(data_1st_t$Guilds)) >= param)
data_1st_list_p$wonder_sideA    <- as.integer(data_1st_t$Wonder_side_A == "yes")

data_nowinners_list_p[, "resultado"]  <- 0
data_nowinners_list_p$militar_vitoria <- as.integer(as.integer(as.character(data_nowinners_t$Military_Conflicts_Victory)) >= param)
data_nowinners_list_p$militar_derrota <- as.integer(as.integer(as.character(data_nowinners_t$Military_Conflicts_Defeat)) >= param)
data_nowinners_list_p$tesouro         <- as.integer(as.integer(as.character(data_nowinners_t$Treasury_Contents)) >= param)
data_nowinners_list_p$wonder          <- as.integer(as.integer(as.character(data_nowinners_t$Wonder)) >= param)
data_nowinners_list_p$civil           <- as.integer(as.integer(as.character(data_nowinners_t$Civilian_Structures)) >= param)
data_nowinners_list_p$cientifica      <- as.integer(as.integer(as.character(data_nowinners_t$Scientific_Structures)) >= param)
data_nowinners_list_p$comercial       <- as.integer(as.integer(as.character(data_nowinners_t$Commercial_Structures)) >= param)
data_nowinners_list_p$guilda          <- as.integer(as.integer(as.character(data_nowinners_t$Guilds)) >= param)
data_nowinners_list_p$wonder_sideA    <- as.integer(data_nowinners_t$Wonder_side_A == "yes")

#junta tudo para uma matriz completa
full_datalist_param <- rbind(data_1st_list_p, data_nowinners_list_p)
...
write.csv(full_datalist_param, "7wonders_assoc_param.csv")
...
for(i in 1:10){
  full_datalist_param[,i] <- factor(full_datalist_param[,i])
...
rules_param <- apriori(full_datalist_param, parameter = list(target = "rules", conf = 0.2, supp = 0.2, minlen = 2))
rules_media <- apriori(full_datalist_media, parameter = list(target = "rules", conf = 0.2, supp = 0.2, minlen = 2))
rules_1quartil <- apriori(full_datalist_1quartil, parameter = list(target = "rules", conf = 0.2, supp = 0.2, minlen = 2))
rules_3quartil <- apriori(full_datalist_3quartil, parameter = list(target = "rules", conf = 0.2, supp = 0.2, minlen = 2))

rules_sub_param <- subset(rules_param, subset = rhs %in% "resultado=1" | lhs %in% "resultado=1")
...
write(rules_param, file = "rules_param_7players.csv", sep = ';', quote = FALSE, row.names = FALSE)
write(rules_media, file = "rules_media_7players.csv", sep = ';', quote = FALSE, row.names = FALSE)
...
#write(rules_sub_param, file = "rules_sub_param_3players.csv", sep = ';', quote = FALSE, row.names = FALSE)
#write(rules_sub_media, file = "rules_sub_media_3players.csv", sep = ';', quote = FALSE, row.names = FALSE)
#write(rules_sub_1quartil, file = "rules_sub_1quartil_3players.csv", sep = ';', quote = FALSE, row.names = FALSE)
#write(rules_sub_3quartil, file = "rules_sub_3quartil_3players.csv", sep = ';', quote = FALSE, row.names = FALSE)
